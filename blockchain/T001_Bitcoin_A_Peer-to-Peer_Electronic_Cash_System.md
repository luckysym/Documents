...
&emsp;&emsp;我们考虑一个场景，一个攻击者尝试用比正常区块链更快的生成速度产生一个替代区块链。就算能过做到，也并不代表这个系统能够被任意改变，比如凭空创建币值，或者取得从来不属于他的钱。网络节点不会接受一个无效的支付交易，诚实的节点永远不会接受一个包含无效交易的区块。攻击者能做的仅仅是尝试改变他自己的交易并拿回他已经付出去的钱。

&emsp;&emsp;诚实链与攻击链的竞争，具有Binomial Random Walk的特征。成功的事件是诚实链被一个区块所扩展，并递增它的lead值+1。失败的事件是攻击者的区块链被一个区块所扩展，与诚实链的差距减少1。

&emsp;&emsp;攻击者从一个既定的差额点追赶上的概率，是一个类似乎赌徒破产问题（Gambler's Ruin）。假定一个拥有无限信用的赌徒，但出于亏损状态，试图通过无限次的赌博来翻本。我们可以计算出他翻本的概率，即一个攻击者追上诚实链的概率。

## 12. 结论

&emsp;&emsp;我们已经提出了一个不用依赖信用的电子交易系统。我们实现了这个基于电子签名的一般货币框架，提供了对所有权的强有力控制，但是并没有解决双重花费问题（Double-Spending）。为了解决这个问题，我们提出了一个使用工作朗证明记录交易历史的P2P网络，只要诚实的网络节点控制的绝大多数的计算能力，这些交易迅速的变为在计算理论上一个攻击者无法篡改的数据。该网络健壮性来源于非结构化和简单性。网络节点同时运行但只有少量协作。由于消息不需要被发送到任何特定的位置，仅仅需要尽快传递出去，因此不需要进行验证。节点在网络中可以任意下线和上线，并在上线后接受网络中的工作量证明数据链，里面记录了在节点下线后的交易信息。它们使用CPU计算能力进行投票，表达它们所对某个有效区块的接受，并致力于延长这些数据链，同时拒绝无效的数据链。在这种共识机制下，可以强化任何需要的规则和奖励制度。
